...
export default function Block({ items, title }: BlockProps) {
    ...
    return (
        <Transition className={`relative wrap ${classes.wrap}`} fade={false} stall={0.1}>
            <h3
                className={`flex leading-none pl-4 relative text-xl tracking-widest uppercase sm:pl-5 md:pl-6 ${classes.title}`}
            >
                <span>{title}</span>
