...
export default function HeaderName() {
    ...
    const topClip = new Array(10).fill(0).map((_item, index) => ({
        x: 9 * index + '%',
        y: 40 * (index % 2) + '%',
    }));

    const bottomClip = new Array(10).fill(0).map((_item, index) => ({
        x: 9 * index + '%',
        y: 10 * (index % 2) + 80 + '%',
    }));
    ...
    return (
        <h1 className="text-2xl xs:text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl">
            <Transition className="block" fade={false} stall={1}>
                <svg className={classes.svg}>
                    ...
                    <g>
                        <text className="font-light tracking-huge uppercase" clipPath="url(#top-clip)" x="-1%" y="55%">
                            Emre Koc
                        </text>
                        <text
                            className={`font-semibold tracking-huge uppercase ${classes.bottom}`}
